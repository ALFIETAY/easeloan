...
async function createLoanApplication(transaction){
    ...
    // Set Status to 'Application Received'
    const setStatus = factory.newResource(namespace, 'Status', transaction.applicationId);
    setStatus.status = 'Application Received';
    setStatus.currentOrganisation = 'Bank';
    setStatus.date = new Date().toLocaleString();
    const statusRegistry = await getAssetRegistry(setStatus.getFullyQualifiedType());
    await statusRegistry.add(setStatus);
...
let	maxLoanAmount = 0
  ...
  loanContract.status = factory.newRelationship(namespace, 'Status', reviewLoanContract.application.applicationId);

  // Set Status to 'Loan Contract Generated'
  const statusRegistry = await getAssetRegistry(namespace + '.Status');
  const updateStatus = await statusRegistry.get(application.applicationId);
  updateStatus.currentOrganisation = 'Bank';
  updateStatus.status = 'Loan Contract Generated';
...
async function LoanCompletion(loanConfirmation){
  ...
  if (loanConfirmation.loanContractStatus === 'COMPLETED') {
    ...
    // Set Status to 'Mortgage Loan Completed'
    const statusRegistry = await getAssetRegistry(namespace + '.Status');
    const updateStatus = await statusRegistry.get(loanContract.status.applicationId);
    updateStatus.status = 'Mortgage Loan Confirmed';
    updateStatus.date = new Date().toLocaleString();
    await statusRegistry.update(updateStatus);
    ...
    const statusRegistry = await getAssetRegistry(namespace + '.Status');
    const updateStatus = await statusRegistry.get(loanContract.status.applicationId);
    updateStatus.status = 'Mortgage Loan Rejected';
    updateStatus.date = new Date().toLocaleString();
    await statusRegistry.update(updateStatus);
