...
async function createCreditReport(transaction){
    ...
    creditReport.status = 'AWAITING_APPLICATION';
...
async function updateCreditReport(transaction){
    ...
    if (transaction.status === 'AWAITING_APPLICATION') {
...
async function createValuationReport(transaction){
    ...
    valuationReport.status = 'AWAITING_APPLICATION';
...
async function createTitleRegistry(transaction){
    ...
    titleRegistryReport.status = 'AWAITING_APPLICATION';
...
/**
 * Smart Contract checking on Loan Contract Information
...
async function LoanContract(reviewLoanContract){
    ...
    const loanContract = factory.newResource(namespace, 'LoanContract', 'LC0001');
    loanContract.applicantName = application.applicantName;
    loanContract.loanTenure = loanTenure;
    loanContract.loanAmount = maxLoanAmount;
